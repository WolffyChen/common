...
import sys
sys.path.insert(0, "/home/wolffy/chen/caffe/python")
import caffe
...
def merge_conv_and_bn(net, i_conv, i_bn, i_scale):
    assert (i_conv is not None)
    assert (i_bn is not None)
...
def merge_batchnorms_in_net(net):
    ...
    for i, layer in enumerate(net.layers):
        ...
        assert (conv_layer_name and conv_layer_top_name)
...
def process_model(net, net_map, src_model, dst_model):
    ...
    new_model.name = model.name + '_nobn' if model.name != '' else 'deploy_nobn'
...
def main(args):
    ...
    if osp.exists(args.model + '.temp.pt'):
...
if __name__ == '__main__':
    parser = ArgumentParser(description="Generate Batch Normalized model for inference")
